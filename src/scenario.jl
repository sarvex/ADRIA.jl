...
function run_model(domain::Domain, param_set::NamedDimsArray, corals::DataFrame, cache::NamedTuple)::NamedTuple
    ...
    if is_guided
        ...
        tolerances = (iv__coral_cover=(>, param_set("iv__coral_cover__tol") .* area_to_seed),
            iv__heat_stress=(<, param_set("iv__heat_stress__tol")),
            iv__wave_stress=(<, param_set("iv__wave_stress__tol")))

        rankings, criteria_store, thresholds_store, min_distance = initialize_mcda(domain, param_set, depth_priority, tolerances)
    end
    ...
    # debug_log = zeros(tf, n_locations)
    # rec_log = zeros(tf, 6, n_locations)
    # juv_log = zeros(tf, 12, n_locations)
    ...
    @inbounds for tstep::Int64 in 2:tf
        ...
        if (fogging > 0.0) && in_shade_years && (has_seed_locations || has_shade_locations)
            if has_seed_locations
                # Always fog seeded locations if they are selected
                location_locs::Vector{Int64} = prefseedlocations
            elseif has_shade_locations
                # Use locations selected for fogging otherwise
                location_locs = prefshadelocations
            end
        ...
        @. tmp = ((Y_pstep * prop_loss) * total_location_area) / absolute_k_area
        ...
        # Using the last step from ODE above, proportionally adjust location coral cover
        # if any are above the maximum possible (i.e., the location `k` value)
        # debug_log[tstep, :] = sum(sol.u[end] .* absolute_k_area ./ total_location_area, dims=1)
        # juv_log[tstep, :, :] = (sol.u[end].*absolute_k_area./total_location_area)[juves, :]

        @views Y_cover[tstep, :, :] .= sol.u[end] .* absolute_k_area ./ total_location_area
