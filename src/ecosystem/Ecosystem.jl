...
Base.@kwdef struct Intervention{N,P,N2,P2} <: EcoModel
    ...
    seed_freq::N = Param(5, ptype="integer", bounds=(0.0, 5.0 + 1.0), dists="unif",
        name="Seeding Frequency", description="Frequency of seeding location selection (0 is set and forget).")
    shade_freq::N = Param(1, ptype="integer", bounds=(0.0, 5.0 + 1.0), dists="unif",
        name="Shading Frequency", description="Frequency of shading location selection (0 is set and forget).")
...
Base.@kwdef struct Criteria{P,N} <: EcoModel
    iv__wave_stress__seed_shade::P = Param(1.0, ptype="real", bounds=(0.0, 1.0), dists="unif",
    ...
    iv__heat_stress__seed_shade::P = Param(1.0, ptype="real", bounds=(0.0, 1.0), dists="unif",
    ...
    iv__in_connectivity__shade::P = Param(0.0, ptype="real", bounds=(0.0, 1.0), dists="unif",
    ...
    iv__coral_cover__shade::P = Param(0.0, ptype="real", bounds=(0.0, 1.0), dists="unif",
    ...
    iv__priority__shade::P = Param(0.0, ptype="real", bounds=(0.0, 1.0), dists="unif",
    ...
    top_n::N = Param(10, ptype="integer", bounds=(5.0, 50.0 + 1.0), dists="unif",
        name="Top N", description="Replaces a given deployment location with a top-ranked location if it does not satisfy the minimum distance threshold.")
    depth_min::P = Param(5.0, ptype="real", bounds=(3.0, 5.0), dists="unif",
        name="Minimum Depth", description="Minimum depth for a location to be included for consideration.\nNote: This value will be replaced with the shallowest depth value found if all locations are found to be deeper than `depth_min + depth_offset`.")
